...
export const ExploreSection: React.FC<ExploreSectionProps> = ({
  ...
  return (
    <div className="px-4 py-4 max-w-md mx-auto text-black">
      {/* Retro Postcard Header in Mango Popsicle Palette with Black Text */}
      ...
      {!loading && places.length === 0 && (
        <div className="text-center py-10 bg-[#F2B949] rounded-2xl border-3 border-black p-6 shadow-retro text-black">
          ...
          <p className="text-xs text-black font-mono mt-1 font-medium">Try another category to discover more landmarks.</p>
          <button
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 cursor-pointer" onClick={() => onSelectDetail(p.id, 'explore')}>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {p.entry_fee && p.entry_fee.toLowerCase().includes("free") && (
                    <span className="text-[9px] font-black px-1.5 py-0.5 bg-[#F2E829] text-black border border-black rounded">
                      FREE ENTRY
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-3 mt-2 text-[11px] font-mono">
                  <span className="text-black font-black">★ {p.rating} ({p.review_count || 50})</span>
                  <span className="text-black font-bold">🕒 {p.best_time || "Morning / Sunset"}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <button
                  ...
                  className="p-2 bg-[#F27430] text-black rounded-xl border-2 border-black shadow-retro-sm hover:bg-[#F2B949] transition-colors"
                  title="Details"
                >
                  <Navigation size={16} />
                </button>
